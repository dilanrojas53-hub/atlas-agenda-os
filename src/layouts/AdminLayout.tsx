...
const MEMBERSHIP_NAV = [
  { section: 'Operación', items: [
    { tab: 'Solicitudes', icon: <UserCheck size={16} /> },
  ...
  return (
    <div className="admin-shell admin-shell-premium">
      ...
      <aside className={`admin-sidebar ${open ? 'open' : ''}`}>
        <div className="admin-sidebar-head"><Link href={`/${tenant.slug}`} className="admin-brand">Atlas OS</Link><button className="admin-close" onClick={() => setOpen(false)}><X size={18} /></button></div>
        ...
        <nav className="admin-nav">{nav.map(group => <div key={group.section} className="admin-nav-group"><p>{group.section}</p>{group.items.map(item => <button key={item.tab} className={activeTab === item.tab ? 'active' : ''} onClick={() => { onTabChange(item.tab); setOpen(false); }}>{item.icon}<span>{'label' in item ? item.label : item.tab}</span></button>)}</div>)}</nav>
      ...
      <main className="admin-main admin-main-premium">
        <header className="admin-command-center"><div><span className="eyebrow">Panel del negocio · {businessType}</span><h1>{tenant.name}</h1><p>Operación, clientes, pagos y configuración viven dentro de esta zona privada.</p></div><div className="admin-command-actions"><span className="badge badge-amber"><span className="dot dot-amber" /> Activo</span><Link className="btn btn-sm btn-secondary" href={`/${tenant.slug}`}>Vista pública</Link><button className="btn btn-sm btn-secondary" onClick={() => exitZone('admin', tenant.slug, `/${tenant.slug}`)}>Salir</button></div></header>
        {children}
      </main>
    </div>
  );
